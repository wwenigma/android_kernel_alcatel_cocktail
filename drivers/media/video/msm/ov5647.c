...
static int ov5647_d_open(struct inode * inode, struct file * file)
{
	return 0;
}

static int ov5647_d_release(struct inode * inode, struct file * file)
{
	return 0;
}

static int ov5647_d_read(struct file *file, char *buf, size_t count, loff_t * ppos)
{
	unsigned short camid_reg;
	int err;
	int rc = 0;
	...
	rc = ov5647_i2c_read(0x3D05, &camid_reg, 1);
	if (rc < 0)
		printk("xxxxxxxxxxxxxxov5647_d_read: errorxxxxxxxxxxxxx\n");
	else
		printk("xxxxxxxxxxxxxxxxxxxxov5647_d_read: {0x3d05,%d}xxxxxxxxxxxxx\n",camid_reg);

	ov5647_i2c_write(ov5647_client->addr, 0x3D21, 0x00, 2);

	err = copy_to_user(buf,&camid_reg,1);
	if(err)
		return -EAGAIN;
	else
		return 1;
...
static int ov5647_d_ioctl(struct inode * inode, struct file * file, unsigned int cmd, unsigned long arg)
{
	return 0;
}

static struct file_operations ov5647_fops = {
	.owner = THIS_MODULE,
	.open = ov5647_d_open,
	.release = ov5647_d_release,
	.read = ov5647_d_read,
	.ioctl = ov5647_d_ioctl,
};

static struct miscdevice ov5647_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "ov5647_d",
	.fops = &ov5647_fops,
};
//compatibility between Foxconn and truly by allen 20120411 END
		...
		case S_REG_INIT:
			{
				...
				//compatibility between Foxconn and truly by allen 20120411
				 ov5647_i2c_write(ov5647_client->addr, 0x0100, 0x01, 2);
				 ov5647_i2c_write(ov5647_client->addr, 0x3D21, 0x01, 2);
				 udelay(500);

				ov5647_i2c_read(0x3D05, &camid_reg, 1);
				printk("xxxxxxxxxxxxxxxxxxxxov5647_setting: {0x3d05,%d}xxxxxxxxxxxxx\n",camid_reg);
...
static int ov5647_i2c_remove(struct i2c_client *client)
{
	if(misc_deregister(&ov5647_device))
	{
		printk("XXXXXXXXXXXXXXov5647_device & misc_deregister fail XXXXXXXXXXXXXXXXXX\n");
	}
	return 0;
}
...
static int ov5647_i2c_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	//compatibility between Foxconn and truly by allen 20120411
	if(misc_register(&ov5647_device))
	{
		printk ("xxxxxxxxxxxxxxxxxxxxov5647misc_device register failedxxxxxxxxxxxxxxxxxxxx\n");
