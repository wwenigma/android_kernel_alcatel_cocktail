...
volatile char exit_poweroff_charging = 0;
EXPORT_SYMBOL(exit_poweroff_charging);
#endif
...
int power_off_charging_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	switch(cmd) {
		case 0:
#ifndef TARGET_BUILD_MMITEST
			if(power_off_charging_state == 1){
				exit_poweroff_charging = 1;
			}
#endif
...
struct file_operations power_off_charging_fops =
{
	...
	.ioctl = &power_off_charging_ioctl,
};
