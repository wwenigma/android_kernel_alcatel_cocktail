...
#include <linux/earlysuspend.h>  //joey
...
#undef NFC_ACK_SLEEP
extern int register_reboot_notifier(struct notifier_block *nb);//joey add for  ven pull down when power off
extern int unregister_reboot_notifier(struct notifier_block *nb);//joey add for  ven pull down when power off
...
struct pn544_dev {
	...
	//struct early_suspend early_suspend;   //joey
...
//joey add for  ven pull down when power off
...
#ifdef NFC_ACK_SLEEP
//joey  add ,enable irq when sleep
//static void pn544_early_suspend(struct early_suspend *h)
		...
		const struct i2c_device_id *id)
{
	...
	//joey
   /*
        pn544_dev->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB;
	pn544_dev->early_suspend.suspend = pn544_early_suspend;
	pn544_dev->early_suspend.resume = pn544_late_resume;
	register_early_suspend(&pn544_dev->early_suspend);
   */
	return 0;
#ifdef NFC_ACK_SLEEP	
err_irq_wake:  //joey  add ,enable irq when sleep
...
static struct i2c_driver pn544_driver = {
	...
	.remove		= pn544_remove,
#ifdef NFC_ACK_SLEEP	
	.suspend       = pn544_suspend,  //joey  add ,enable irq when sleep
	.resume         = pn544_resume,
#endif	
	.driver		= {
		.owner	= THIS_MODULE,
...
static int __init pn544_dev_init(void)
{
	...
	if (!client){
        ...
        register_reboot_notifier(&pn544_notifier);//joey add for  ven pull down when power off
	return 0;
...
static void __exit pn544_dev_exit(void)
{
	...
	unregister_reboot_notifier(&pn544_notifier);//joey add for  ven pull down when power off
