...
#define PMIC_GPIO_CAM_FLASH_INT 25
...
#if defined(CONFIG_MPU_SENSORS_MPU3050) || defined(CONFIG_MPU_SENSORS_MPU3050_MODULE)
#define Gyro_GPIO_INT 44
#endif

/* maybe remove after proto */
//#define GYRO_USE_I2C9
...
static int pm8058_gpios_init(void)
{
	...
	struct pm8058_gpio cam_flash_int = {
		.direction      = PM_GPIO_DIR_IN,
		.pull           = PM_GPIO_PULL_NO,
	...
	rc = pm8058_gpio_config(PMIC_GPIO_CAM_FLASH_INT - 1, &cam_flash_int);
	if (rc) {
	...
	if (machine_is_msm7x30_cocktail()) {
/*if is cocktail debug*/
		rc = pm8058_gpio_config(PMIC_GPIO_SDC4_EN_N, &sdc4_en);
		if (rc) {
			pr_err("%s PMIC_GPIO_SDC4_EN_N config failed\n",
								 __func__);
...
/*	KEY(0, 0, KEY_Q),
	...
	KEY(2, 4, KEY_P),
	KEY(2, 5, KEY_DOWN),
	KEY(2, 6, KEY_ENTER), //this is center key 2045
...
#ifdef CONFIG_MACH_MSM7X30_COCKTAIL
//TCL-Cocktail use VREG_VCM_VDD for VCM power
...
static struct vreg *verg_camera_LDO; //wangdong add for config LDO
static int config_camera_on_gpios(void)
{
	//wangdong add for config LDO start
	int rc = -EINVAL;
	verg_camera_LDO = vreg_get(NULL,"wlan");
	if(!verg_camera_LDO){
		pr_err("%s: verg_camera_LDO get failed \n", __func__);
		return rc;
	}
	rc = vreg_set_level(verg_camera_LDO,1500);
	if(rc){
		pr_err("%s: verg_camera_LDO set failed \n", __func__);
		goto l13_put;
	}
	rc = vreg_enable(verg_camera_LDO);
	if(rc){
		pr_err("%s: verg_camera_LDO enable failed \n", __func__);
		goto l13_put;
	}
	//wangdong add for config LDO end
...
static void config_camera_off_gpios(void)
{
	//wangdong add for config LDO start
	int rc;
	if (IS_ERR(verg_camera_LDO)) {
		pr_err("%s: vreg_get(%s) failed (%ld)\n",
		__func__, "verg_camera_LDO", PTR_ERR(verg_camera_LDO));
		return;
	}
	rc = vreg_disable(verg_camera_LDO);
	if (rc)
		pr_err("%s: vreg_disable(verg_camera_LDO) failed (%d)\n", __func__, rc);
		...
		only configure it for non-Timpani RF card */
	{
		...
		pr_err("%s: camera_off_vcm_gpio_table\n",__func__);
	}
...
struct msm_camera_device_platform_data msm_camera_device_data = {
	...
	.ioext.clkctl = 0xAB800000,
	.ioext.clkctlsz  = 0x00000400,
#endif
...
static struct msm_camera_sensor_info msm_camera_sensor_ov8820_data = {
        ...
        .sensor_reset   = 21, //CAM_5MP_RESET
        .sensor_pwd     = 143, //MIPI_CAM_PDN
        .vcm_pwd        = 19,  //VREG_VCM_VDD   <---> VREG_L13 <--->vreg_wlan1
        .vcm_enable     = 19,
        .pdata          = &msm_camera_device_data,
        .resource       = msm_camera_resources,
        .num_resources  = ARRAY_SIZE(msm_camera_resources),
...
static struct platform_device msm_camera_sensor_ov8820 = {
...
static struct android_usb_product usb_products[] = {
	...
	//set default pid as 9025, adb is on.
	//meeting the NPI's requirement.
	{
		.product_id	= 0x9025,
...
static struct usb_mass_storage_platform_data cdrom_pdata = {
	.nluns		= 1,
	.vendor		= "Qualcomm Incorporated",
	.product        = "CDROM",
	.release	= 0x0100,
...
static struct platform_device usb_cdrom_device = {
	.name	= "usb_cdrom",
	.id	= -1,
	.dev	= {
...
static int hdmi_comm_power(int on, int show)
{
	int rc = gpio_set("gp9", "LDO12", 3000, on);
	if (rc)
		pr_warning("hdmi_comm_power: LDO12 gpio failed: rc=%d\n", rc);
	if (show)
...
static struct msm_gpio gyro_gpio_int_config_data[] = {
	{ GPIO_CFG(44, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "gyro_int" },
};

static void gyro_init_irq(void)
{
	int rc = msm_gpios_enable(gyro_gpio_int_config_data,
			ARRAY_SIZE(gyro_gpio_int_config_data));
	if (rc < 0) {
		pr_err("%s: gpio enable failed: %d\n", __func__, rc);
		return;
	}
...
//the devices on the simulative i2c which adapter is 9. They depend on CONFIG_I2C_GPIO
static struct i2c_board_info gyro_i2c_board_info[] = {
#if defined(CONFIG_MPU_SENSORS_MPU3050) || defined(CONFIG_MPU_SENSORS_MPU3050_MODULE)
	{
		I2C_BOARD_INFO(SENSOR_MPU_NAME, 0x68),
		.irq = MSM_GPIO_TO_INT(Gyro_GPIO_INT),
...
static unsigned wega_reset_gpio =
	GPIO_CFG(GPIO_RESET_N, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_16MA);
...
static int display_common_power(int on)
{
	...
	struct vreg *vreg_ldo15 = NULL, *vreg_ldo20 = NULL;
	...
	gpio_set_value(GPIO_RESET_N, 0);
	
	if (on) {							
		rc = vreg_enable(vreg_ldo15);
		rc = vreg_enable(vreg_ldo20);
		gpio_set_value(GPIO_RESET_N, 1);
		mdelay(5);	
	} else {			
...
static struct msm_gpio lcd_sharp_panel_gpios[] = {
	...
	{ GPIO_CFG(95, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_grn3" },
	{ GPIO_CFG(96, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_grn4" },
#ifndef CONFIG_I2C_GPIO_COCKTAIL_GPIO9 
	{ GPIO_CFG(97, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_grn5" },
	{ GPIO_CFG(98, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_grn6" },
#endif //TCL
	{ GPIO_CFG(99, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "lcdc_grn7" },
...
int bcm_wlan_power(int val)
{
	...
	if (val) {
		...
		msm_sdcc_setup_gpio(3, 0);
	}
		mdelay(100);
		gpio_set_value(WL_REG_ON, val);
		mdelay(250);
	gpio_free(WL_REG_ON);
	return 0;
}

int bcm_wlan_reset(int val)
{
...
int bcm_wlan_set_carddetect(int val)
{
...
static struct wifi_platform_data msm_wifi_control = {
    .set_power      = bcm_wlan_power,
    .set_reset      = bcm_wlan_reset,
    .set_carddetect = bcm_wlan_set_carddetect,
...
#ifdef CONFIG_BCM4330_POWER
static struct msm_gpio wifi_config_power_on[] = {
	{ GPIO_CFG(43, 	0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,   GPIO_CFG_2MA),	"WL_REG_ON" }
};

//static struct msm_gpio wifi_wake[] = { //harry
//	{GPIO_CFG(42,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "wifi_wake_bb"},
//};
...
static void __init wlan_power_init(void)
{
	...
	rc = gpio_request(WL_REG_ON, "wl_reg_on");

	rc = msm_gpios_enable(wifi_config_power_on, ARRAY_SIZE(wifi_config_power_on));

	if (rc < 0) {
		return;
	}
	//msm_gpios_enable(wifi_wake, ARRAY_SIZE(wifi_wake));//harry
...
extern void bcm_detect_card(int);
static uint32_t msm_sdcc_setup_gpio(int, unsigned int);
...
static struct msm_gpio qup_i2c_gpios_io[] = {
	...
	{ GPIO_CFG(17, 0, GPIO_CFG_OUTPUT|GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "qup_sda" },
};
static struct msm_gpio qup_i2c_gpios_hw[] = {
	{ GPIO_CFG(16, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "qup_scl" },
	{ GPIO_CFG(17, 2, GPIO_CFG_OUTPUT|GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), "qup_sda" },
};
...
qup_i2c_gpio_config(int adap_id, int config_type)
{
	...
	if (adap_id != 4)
		return;
	printk(KERN_ERR "liuli %s config_type %d \n", __func__, config_type);
	if (config_type)
...
static struct msm_gpio sdc3_cfg_data[] = {
	...
	{GPIO_CFG(42,  0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA), "wifi_oob"}, //harry
};
...
static struct msm_gpio sdc3_sleep_cfg_data[] = {
	...
	{GPIO_CFG(119, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
			"sdc3_dat_0"},
	{GPIO_CFG(42,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA), 
			"wifi_oob"}, //harry
};
...
static int cocktail_enable_sdc4(int enable)
{
	int gpio_value;
	int rc=0;
	/*low means vdd high*/
...
/*we have already request on init*/
/*
	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_SDC4_EN_N),
			  "sdc4_en");
	if (rc) {
...
*/
	//gpio_value=0; /*Only for debug SD, zzk 0927. */
	gpio_set_value_cansleep(
		PM8058_GPIO_PM_TO_SYS(PMIC_GPIO_SDC4_EN_N), gpio_value);
...
static uint32_t msm_sdcc_setup_vreg(int dev_id, unsigned int enable)
{
	...
	/* sdc3 poweron on another branch */
	if (dev_id == 3)
		return rc;
	...
	if(dev_id==4){
		/*ljm ignore sdc4 vreg,we use gpio control instead*/ 
		return 0;
	}
	if (enable) {
...
//	printk("XXXXXXXXXXXXXXXXXXXxx msm_sdcc_setup_power %d XXXXXXXXX\n", vdd);
	...
	{
		/*cocktail sdc4 en disable,cocktail don't use vreg to control it,we ignore it,just make compiler happy*/
		rc = msm_sdcc_setup_vreg(pdev->id, (vdd ? 1 : 0));
		cocktail_enable_sdc4(vdd ? 1 : 0);
	}
out:
	return rc;
}
...
static struct mmc_platform_data msm7x30_sdc3_wlan_data = {
...
static void __init msm7x30_init_mmc(void)
{
	...
	msm_sdcc_setup_gpio(3, 1);
	msm_add_sdcc(3, &msm7x30_sdc3_wlan_data);
#endif
#ifdef CONFIG_MMC_MSM_SDC4_SUPPORT
...
static struct msm_gpio uart2_dml_config_data[] = {
	{ GPIO_CFG(85, 3, GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "UART2DM_Rx"},
	{ GPIO_CFG(87, 3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "UART2DM_Tx"},
	{ GPIO_CFG(88, 2, GPIO_CFG_INPUT,   GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "UART2DM_CTS"},
	{ GPIO_CFG(89, 3, GPIO_CFG_OUTPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "UART2DM_RFR"},
};

static void msm7x30_init_uart2_dml(void)
{
	msm_gpios_request_enable(uart2_dml_config_data,
			ARRAY_SIZE(uart2_dml_config_data));
...
#else
        i2c_register_board_info(4 /* QUP ID */, gyro_i2c_board_info,
                                 ARRAY_SIZE(gyro_i2c_board_info));
#endif /* GYRO_USE_I2C9 */
...
#ifdef GYRO_USE_I2C9
	i2c_register_board_info(9, gyro_i2c_board_info,
			ARRAY_SIZE(gyro_i2c_board_info));
#else
    i2c_register_board_info(9, msm_camera_boardinfo,
            ARRAY_SIZE(msm_camera_boardinfo));
#endif /* GYRO_USE_I2C9 */
...
MACHINE_START(MSM7X30_COCKTAIL, "QCT MSM8X55 Cocktail")
